...
SuperBlock.prototype = {
  ...
  parse: function( buffer ) {
    ...
    this.uuid = Buffer.from( buffer.slice( 0x068, 0x078 ) )
    ...
    this.uuid = Buffer.from( buffer.slice( 0x0D0, 0x0E0 ) )
    ...
    this.mmpBlock = uint64.readLE( buffer, 0x168 )
    ...
    this.kbytesWritten = uint64.readLE( buffer, 0x178 )
    this.snapshotInum = buffer.readUInt32LE( 0x180 )
    this.snapshotID = buffer.readUInt32LE( 0x184 )
    this.snapshotRBlocksCount = uint64.readLE( buffer, 0x188 )
    ...
    this.firstErrorBlock = uint64.readLE( buffer, 0x1A0 )
    ...
    this.lastErrorBlock = uint64.readLE( buffer, 0x1D8 )
    ...
    this.encryptSalt = Buffer.from( buffer.slice( 0x258, 0x268 ) )
